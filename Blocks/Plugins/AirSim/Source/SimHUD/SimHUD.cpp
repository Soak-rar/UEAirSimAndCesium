...
#ifndef ClientControlWidget
#define ClientControlWidget
#include "ClientControlWidget.h"
#endif // !ClientControlWidget
...
void ASimHUD::createClientForThread()
{
    ClientRunable* client = new ClientRunable(FName("MyClient!!!"));

    ClientThread = FRunnableThread::Create(client, *FString("ClientThread"));
}

void ASimHUD::createClientControlWidget()
{
    APlayerController* player_controller = this->GetWorld()->GetFirstPlayerController();

    if (player_controller) {
        TSubclassOf<UUserWidget> ClientControlWidgetClass = LoadClass<UUserWidget>(player_controller, TEXT("/AirSim/Blueprints/BP_ClientWidget.BP_ClientWidget_C"));
        if (ClientControlWidgetClass) {
            client_control_widget = CreateWidget<UClientControlWidget>(GetWorld(), ClientControlWidgetClass);
            if (client_control_widget)
            {
                client_control_widget->init_(this);
                UScaleBox* box = (UScaleBox*)widget_->GetWidgetFromName("SubWindow0ScaleBox_1");
                if (box)
                {
                    box->SetVisibility(ESlateVisibility::Visible);
                    box->AddChild(client_control_widget);
                }
            }
        }
    }
...
uint32 ClientRunable::Run()
{
    UE_LOG(LogTemp, Warning, TEXT("FMyRunnable Run: %s sleep for 5 s"), *client_name.ToString());
    FPlatformProcess::Sleep(5);
    
    UE_LOG(LogTemp, Warning, TEXT("FMyRunnable Run: %s"), *client_name.ToString());
    client->enableApiControl(true);
    client->takeoffAsync(20, "Drone1")->waitOnLastTask();
    return 0;
}

void ClientRunable::Stop()
{
    UE_LOG(LogTemp, Warning, TEXT("FMyRunnable Stop: %s"), *client_name.ToString());
}

void ClientRunable::Exit()
{
    UE_LOG(LogTemp, Warning, TEXT("FMyRunnable Exit: %s"), *client_name.ToString());
}
